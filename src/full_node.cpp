...
full_node::full_node(const configuration& configuration)
  : multi_crypto_setter(configuration.network),
    ...
    keoken_settings_(configuration.node)
{}

full_node::~full_node()
{
    // LOG_INFO(LOG_KEOKEN) << "full_node::~full_node()";
...
void full_node::start(result_handler handler)
{
    ...
    if (!chain_.start())
    {
        LOG_ERROR(LOG_KEOKEN) << "Failure starting blockchain.";
        handler(error::operation_failed);
        return;
    }
...
{
    return attach<session_block_sync>(hashes_, chain_, keoken_settings_);
}
...
{
    // LOG_INFO(LOG_KEOKEN) << "full_node::close()";
...
const keoken::settings& full_node::keoken_settings() const
{
    return keoken_settings_;
}
